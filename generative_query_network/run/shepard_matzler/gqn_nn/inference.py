...
class Core(nn.Module):
    def __init__(self, channels_chz=64, input_channel=64*2+32+7+256):
        super(Core, self).__init__()
        # initialW=HeNormal(0.1)
        # input_channel = self.channels_chz=64 + self.channels_chz=64 + self.inference_channels_map_x=32 + v=7 + self.channels_r=256
    ...
    def forward(self, prev_hg, prev_he, prev_ce, x, v, r):
        v = torch.reshape(v, v.shape + (1, 1))
        v = v.repeat((1,1,)+ prev_he.shape[2:])

        lstm_in = torch.cat((prev_he, prev_hg, x, v, r), dim=1)
        forget_gate = torch.sigmoid(self.lstm_f(lstm_in))
        input_gate = torch.sigmoid(self.lstm_i(lstm_in))
        next_c = forget_gate * prev_ce + input_gate * torch.tanh(self.lstm_tanh(lstm_in))
...
class Downsampler(nn.Module):
    def __init__(self, channels=32):
        ...
        self.conv_x_1 = nn.Conv2d(3, channels // 2, kernel_size=2, stride=2, padding=0)
        self.conv_x_2 = nn.Conv2d(channels // 2, channels, kernel_size=2, stride=2, padding=0)

    def forward(self, x):
        x = F.relu(self.conv_x_1(x))
        x = self.conv_x_2(x)
        return x